---
title: Simplified test suites (DSL)
description: "Easily create test suites for TESTed using the DSL."
---

# Simplified test suites (DSL)

::: warning Warning
The DSL for test suites is still in development and thus experimental.
This reference is thus also in draft status.
:::
...
- With a domain-specific language: easier, but more limited. This is what we describe here.
- With JSON: complexer, but more flexible. This is described on [this page](../json).

The most important goal of the domain-specific language
([DSL](https://en.wikipedia.org/wiki/Domain-specific_language)) is to make specifying test suites as easy as possible.
This sometimes results in less flexibility compared to the full format.
The [80/20 rule](https://en.wikipedia.org/wiki/Pareto_principle) applies:
we aim that 80% of test suites can be defined with the DSL, while the full format is needed for the remaining 20%.

The DSL for test suites is written in [YAML](https://en.wikipedia.org/wiki/YAML).
The structure of the DSL is very similar to the structure of the tests used in Dodona.
The following code fragment describes the structure of the DSL-test suites.
...
We now describe the various aspects of this format using examples.

## Input-output exercises

Input-output exercises are exercises where the input to the submission is given via stdin, and the result must be written to stdout.
These kinds of exercise are often used in programming contests.

A good example is an "echo" exercise: a submission must read data from stdin and write that data to stdout.
The test suite then becomes:
...
The following figure shows how the feedback from the previous test suite is shown in Dodona.
The figure has been augmented with annotations of the form `[0].testcases[0].stdin`.
For example, `[0]` denotes the first tab, `contexts[0]` the first context,
`stdin` the value given to stdin and `stdout` the expected stdout.
...
The structure of the test suites is a list at the highest level, which contains tab-objects.
...
For this output, we recommend using strings.
...
We will now extend the test suite with multi-line strings and multiple tabs.
...
#### Multi-line
As visible in the test suite from the codefragment, YAML has multiple ways to describe multi-line strings.
...
Therefore can the user disable these optimizations for the entire test suite, should this be necessary.
...
#### Textual types
The second testcase in the test suite for __How smart are you?__, illustrates that not only strings can be used for standard input, output or error.
...
Now we will examine a test suite for simple function calls.
The test suite that we will examine is for the exercise [Rail fence cipher](https://dodona.ugent.be/en/courses/392/series/3922/activities/444829407/).
...
We will now look at a test suite where we also assign values to a variable.
...
You can specify the expected error messages in test suites, not to be confused with error types (which are programming language dependent).
For this we are looking at a test suite for the "division" function.
...
We have already seen test suites for both input-output or function calls.
We will now combine these concepts in one test suite.
We will use the basic calculator example for this test suite.
...
## Convert
The canonical format for TESTed test suites is the full JSON format. However, TESTed converts the DSL internally to the JSON format, so you can use the DSL directly.
...
$ python3 -m tested.translate_dsl < testsuite.yaml > testsuite.json
# Short options
$ python3 -m tested.translate_dsl -i testsuite.yaml -o testsuite.json
# Long option names
$ python3 -m tested.translate_dsl --dsl testsuite.yaml --json testsuite.json
