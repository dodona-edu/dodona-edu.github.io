...
class Image {
  ...
  async drawArrow(x: number, y: number, mirror = false): Promise<void> {
...
type Predicate = (...args: Array<any>) => boolean;
const DEFAULT_PREDICATE: Predicate = () => true;
...
class Wizard {
  ...
  constructor(baseUrl: string, imageFolder: string, fileExtension: string) {
    this.browser = null;
    this.page = null;
  ...
  setLanguage(language: string): Wizard {
    this.language = language;
    return this;
  }

  async launch(): Promise<Wizard> {
  ...
  blockElement(selector: string, predicate = DEFAULT_PREDICATE): Wizard {
  ...
  async enterPythonFile(filename: string) {
    ...
    for (const line of content.split('\n')) {
      ...
      for (let i = 0; i < Math.max(indent, prevIndent); i++) {
        await this.press('textarea.ace_text-input', 'Backspace');
      }
      prevIndent = indent;
      await this.typeIn('textarea.ace_text-input', `${line}\n`);
    }
  }
}
...
async function read_submissions(): Promise<number> {
